...
interface TechnicianAiEmptyStateProps {
  onRetry?: () => void
  error?: string | null
}

export default function TechnicianAiEmptyState({ onRetry, error }: TechnicianAiEmptyStateProps) {
  if (error) {
    return (
  ...
  return (
    <div style={{ background: 'rgba(8, 24, 42, 0.72)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '48px 24px', textAlign: 'center', maxWidth: '600px', margin: '40px auto' }}>
      ...
      <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '22px', fontWeight: 700, color: '#ffffff', marginBottom: '8px' }}>
        AI Troubleshooting Workspace Ready
      </h3>
      <p style={{ fontSize: '14px', color: '#94a3b8', lineHeight: 1.6, marginBottom: '24px' }}>
        Type an inverter error code or field anomaly description below to receive instant step-by-step diagnostic analysis and safety protocols.
      </p>
      {onRetry && (
        <button className="btn btn-secondary" onClick={onRetry}>
          <MdRefresh /> Reset Workspace
        </button>
      )}
    </div>
  )
